...
import { addLocalPackageDataWithLicense } from './addLocalPackageDataWithLicense.js';
 ...
 */
const addDependenciesRecursive = async (
  depsIndex,
  projectRootPath,
  exclusions,
  inclusions,
  ...
  do {
    ...
    for (let i = startOfSublist; i <= endOfSublist; i++) {
      ...
      debug(
        `    + ${currentParentPath} -> ${element.name}@${element.installedVersion}`,
      );
      ...
      if (!pathIncludes(currentParentPath, element)) {
        ...
        if (fs.existsSync(elementPackageJsonPath)) {
          ...
          const selectedDependenciesBase = getDependencies(
            packageJson,
            exclusions,
            inclusions,
            element.path,
          ...
          if (selectedDependenciesComplete.length > 0) {
            ...
            selectedDependenciesComplete.forEach((requiredPackage) => {
              if (
                pathIncludes(
                  requiredPackage.path.slice(0, element.path.lastIndexOf('>')),
                  requiredPackage,
                )
              ) {
            ...
            element.requires = selectedDependenciesComplete.sort(
              util.alphaSort,
            );
